...
import sys
class TurboFFT:
    def __init__(self, global_tensor_shape=[256, 1], radix=2, WorkerFFTSizes = [8], threadblock_bs=[1], threadblock_bs_dim=[0], data_type='double2', if_special=False):
        self.fft_code = []
    ...
    def init(self, dim=0):
        self.local_variable = {
            "j" : ("int", "0"), "k" : ("int", "0"), "global_j" : ("int", "0"), "global_k" : ("int", "0"), "data_id" : ("int", "0"),
            "bs_id" : ("int", "0"), "shared_offset_bs" : ("int", "0"), "shared_offset_data" : ("int", "0"),
            "bx": ("int", "gridDim.x - blockIdx.x - 1") if dim == 0 and len(self.WorkerFFTSizes) == 3 else ("int", "blockIdx.x"),
            "tx": ("int", "threadIdx.x"), "offset": ("int", "0"), self.gPtr: (f"{self.data_type}*", "inputs"),
            self.shPtr: (f"{self.data_type}*", "shared"), f"{self.rPtr}[{self.WorkerFFTSizes[dim] + self.ft}]": (self.data_type, None),
            f"{self.rPtr_2}[{self.WorkerFFTSizes[dim]}]": (self.data_type, None), "tmp": (self.data_type, None),
            "tmp1": (self.data_type, None), "angle": (self.data_type, None), "delta_angle": (self.data_type, None), }
...
__global__ void fft_radix_{self.radix}_logN_{int(log(N, self.radix))}_dim_{dim}''' \
    ...
    int bid_cnt = 0; '''
        head += f'''
    int tb_gap = 2 * 108; int delta_bid = ((blockIdx.x / tb_gap) ==  (gridDim.x / tb_gap)) ? (gridDim.x % tb_gap) : tb_gap;
    for(int bid = (blockIdx.x / tb_gap) * tb_gap * 32 + blockIdx.x % tb_gap; bid_cnt < 32 && bid < ({N} * BS + {Ni * threadblock_bs} - 1) / {Ni * threadblock_bs}; bid += delta_bid)'''
        head += '''{bid_cnt += 1;'''
    ...
    def globalAccess(self, dim, global_tensor_shape, threadblock_bs_dim, threadblock_bs, 
        ...
        globalAccess_code = f'''        bx = bid; tx = threadIdx.x;''' 
        ...
        if if_twiddle:
            globalAccess_code += '''global_j = 0; global_k = 0;'''
        access_stride = 1
        for i in blockorder[:-1]:
            stride = max(1, th.prod(global_tensor_shape[:i]))
            if i < dim and if_twiddle:
                globalAccess_code += f'''global_j += (bx % {global_tensor_shape[i] // threadblock_tensor_shape[i]}) * {threadblock_tensor_shape[i]} * {stride};'''
                if i == threadblock_bs_dim:
                    globalAccess_code += f'''global_j += (tx % {threadblock_bs}) * {stride};'''    
            if i == dim:
                if not if_to_shared:
                    
                    globalAccess_code += f'''{self.gPtr} += tx / {threadblock_bs} * {stride};'''
                    access_stride = global_tensor_shape[i] // WorkerFFTSize * stride
                else:
                    globalAccess_code += f'''{self.gPtr} += tx % {global_tensor_shape[i]} * {stride}; shared_offset_data = tx % {global_tensor_shape[i]}; '''         
            ...
            globalAccess_code += f'''{self.gPtr} += (bx % {global_tensor_shape[i] // threadblock_tensor_shape[i]}) * {threadblock_tensor_shape[i]} * {stride};bx = bx / {global_tensor_shape[i] // threadblock_tensor_shape[i]};'''
            if i == threadblock_bs_dim:
                if not if_to_shared:
                    globalAccess_code += f'''{self.gPtr} += tx % {threadblock_bs} * {stride};'''
                else:
                    globalAccess_code += f'''shared_offset_bs = tx / {global_tensor_shape[dim]}; {self.gPtr} += tx / {global_tensor_shape[dim]} * {stride};'''         
        ...
        globalAccess_code += f'''{self.gPtr} += (bx % BS * {th.prod(global_tensor_shape[:-1])});'''
        if if_twiddle:
            globalAccess_code += f'''global_k += tx / {threadblock_bs};'''
        if not if_to_shared:
            for i in range(WorkerFFTSize):
                if not if_output:
                    globalAccess_code += f'''{self.rPtr}[{i}] = *({self.gPtr} + {i * access_stride});'''
                else:
                    if if_twiddle:
                        N = th.prod(global_tensor_shape[:(dim + 1)])
                        if i == 0:
                            globalAccess_code += f'''delta_angle = twiddle[{N - 1} + global_j * ({global_tensor_shape[dim] // WorkerFFTSize})];angle = twiddle[{N - 1} + global_j * global_k];'''                    
                        else:
                            globalAccess_code += f'''tmp = angle; turboFFT_ZMUL(angle, tmp, delta_angle);'''                             
                        globalAccess_code += f'''tmp = {self.rPtr}[{dict_output[i]}]; turboFFT_ZMUL({self.rPtr}[{dict_output[i]}], tmp, angle);'''
                    globalAccess_code += f'''*({self.gPtr} + {i * access_stride}) = {self.rPtr}[{dict_output[i]}]; '''
        else:
            for j in range(access_stride_bs_dim):
                for i in range(access_stride_data_dim):
                    globalAccess_code += f'''bs_id = shared_offset_bs + {int(j * max(1, (T * threadblock_bs) // global_tensor_shape[dim]))};data_id = shared_offset_data + {int(i * ((T * threadblock_bs)))};*({self.shPtr} + (bs_id + data_id) % {global_tensor_shape[dim]} + bs_id * {global_tensor_shape[dim]}) = *({self.gPtr} + {i * access_stride_data + j * access_stride_bs});'''
            globalAccess_code += f'''__syncthreads();'''
            for i in range(WorkerFFTSize):
                globalAccess_code += f'''bs_id = threadIdx.x % {threadblock_bs};data_id = threadIdx.x / {threadblock_bs} + {i * T};{self.rPtr}[{i}] = *({self.shPtr} + (bs_id + data_id) % {global_tensor_shape[dim]} + bs_id * {global_tensor_shape[dim]});'''
    ...
    def shared2reg(self, threadblock_bs, threadblock_tensor_shape, WorkerFFTSize, dim=None):
        shared2reg_code  = ''''''
        access_stride = int(threadblock_bs * th.prod(th.as_tensor(threadblock_tensor_shape)) / WorkerFFTSize)
        ...
        if len(threadblock_tensor_shape) == 2 and len(self.global_tensor_shape) == 2 :
            shared2reg_code += f'''offset = 0;'''
            shared2reg_code += f'''__syncthreads();'''            
        ...
        if dim == 1 and len(self.global_tensor_shape) == 2 :
            shared2reg_code += f'''offset = 0; offset += (tx / {dim_0}) * {dim_0} + ((tx % {dim_0}) / {dim_1}) * {dim_1} + (tx % {dim_1} + tx / {dim_0}) % {dim_1};'''
        else:
            shared2reg_code += f'''offset = 0; offset += tx;'''
        shared2reg_code += '''__syncthreads();'''
        for j in range(WorkerFFTSize + self.ft):
            i = j % WorkerFFTSize
            shared2reg_code += f'''{self.rPtr}[{i}] = {self.shPtr}[offset + {access_stride * i}];'''
        return shared2reg_code
    ...
    def reg2shared(self, threadblock_bs, threadblock_tensor_shape, WorkerFFTSize, dim, dict_output):
        reg2shared_code = '''j = 0;offset  = 0;'''
        if dim == len(threadblock_tensor_shape) - 1:
            access_stride = int(threadblock_bs * th.prod(th.as_tensor(threadblock_tensor_shape))
                         / WorkerFFTSize)
            reg2shared_code += f'''__syncthreads();'''
            for i in range(WorkerFFTSize):
                reg2shared_code += f'''{self.shPtr}[threadIdx.x + {access_stride * i}] = {self.rPtr}[{dict_output[i]}];'''
            reg2shared_code += f'''__syncthreads();'''
        ...
        for i in range(len(bs_tensor_shape)):
            ...
            if i == dim + 1:
                ...
                reg2shared_code += f'''j = tx / {tmp};'''
                continue
            reg2shared_code += f'''offset += ((tx / {tmp}) % {bs_tensor_shape[i]}) * {int(stride / bs_tensor_shape[i])};'''
            tmp *= bs_tensor_shape[i]
            
        if dim == len(threadblock_tensor_shape) - 1:
            access_stride = int(threadblock_bs * th.prod(th.as_tensor(threadblock_tensor_shape)) / self.WorkerFFTSize)
        reg2shared_code += '''__syncthreads();'''
        N = th.prod(th.as_tensor(threadblock_tensor_shape[dim:]))
        for output_id in range(WorkerFFTSize): 
            if dim != len(threadblock_tensor_shape) - 1:
                if output_id == 0:
                    reg2shared_code += f'''delta_angle = twiddle[{N - 1} + j];angle.x = 1;angle.y = 0;'''       
                else:
                    reg2shared_code += f'''tmp = angle;turboFFT_ZMUL(angle, tmp, delta_angle);tmp = {self.rPtr}[{dict_output[output_id]}];turboFFT_ZMUL({self.rPtr}[{dict_output[output_id]}], tmp, angle);'''             
            if dim == 0 and len(self.global_tensor_shape) == 2 :
                reg2shared_code += f'''{self.shPtr}[offset + {access_stride} * ({output_id} + threadIdx.x % {threadblock_tensor_shape[1]}) % {threadblock_tensor_shape[1]} + ({output_id} / {threadblock_tensor_shape[1]}) * {threadblock_tensor_shape[1]}] = {self.rPtr}[{dict_output[output_id]}];'''
            else:
                reg2shared_code += f'''{self.shPtr}[offset + {access_stride * output_id}] = {self.rPtr}[{dict_output[output_id]}];'''
        return reg2shared_code
    ...
    def fft_reg(self, threadblock_bs, threadblock_tensor_shape, WorkerFFTSize, dim, reg_tensor_stride):
        ...
        for i in range(st, logbs - 1, -1):
            for j in range(WorkerFFTSize):
                ...
                rel_bounds = 1e-8
                abs_bounds = 1e-5
                fft_reg_code += f'''tmp = {self.rPtr}[{id_j1}];turboFFT_ZADD({self.rPtr}[{id_j1}], tmp, {self.rPtr}[{id_j2}]);turboFFT_ZSUB({self.rPtr}[{id_j2}], tmp, {self.rPtr}[{id_j2}]);tmp = {self.rPtr}[{id_j2}];'''
                if np.allclose(0, cos(tmp_angle), rel_bounds, abs_bounds):
                    if np.allclose(1, sin(tmp_angle), rel_bounds, abs_bounds):
                        fft_reg_code += f'''{self.rPtr}[{id_j2}].y = tmp.x;{self.rPtr}[{id_j2}].x = -tmp.y;'''
                    else:
                        fft_reg_code += f'''{self.rPtr}[{id_j2}].y = -tmp.x;{self.rPtr}[{id_j2}].x = tmp.y;'''
                    ...
                    if np.allclose(1, cos(tmp_angle), rel_bounds, abs_bounds):
                        ...
                        fft_reg_code += f'''{self.rPtr}[{id_j2}].x = -tmp.x;    {self.rPtr}[{id_j2}].y = -tmp.y;'''
                else:
                    if self.data_type == 'double2':
                        fft_reg_code += f'''angle.x = {cos(tmp_angle)};angle.y = {sin(tmp_angle)};turboFFT_ZMUL({self.rPtr}[{id_j2}], tmp, angle);'''
                    else:
                        fft_reg_code += f'''angle.x = {cos(tmp_angle)}f;angle.y = {sin(tmp_angle)}f;turboFFT_ZMUL({self.rPtr}[{id_j2}], tmp, angle);'''             
        return fft_reg_code
